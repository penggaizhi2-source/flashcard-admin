...
import { Plus, Edit2, Send, Trash2, X, GripVertical, Camera, Users } from 'lucide-react';

// ─── Types ────────────────────────────────────────────────────────────────────

type Step = { id: string; text: string; requiresMedia: boolean };
...
function StepRow({
  ...
  onChange: (id: string, field: keyof Step, value: string | boolean) => void;
  ...
  return (
    <div
      ...
      className={`flex items-start gap-2 p-3 rounded-lg border transition select-none
        ${isBeingDragged ? 'opacity-40' : 'opacity-100'}
        ${isDropTarget ? 'border-blue-400 bg-blue-50' : 'border-gray-200 bg-white'}`}
    >
      <div className="mt-2.5 cursor-grab text-gray-300 hover:text-gray-500 shrink-0">
        <GripVertical size={16} />
      </div>
      <div className="mt-2 w-5 h-5 rounded-full bg-blue-100 text-blue-700 text-[11px] font-bold flex items-center justify-center shrink-0">
        {index + 1}
      </div>
      <textarea
        value={step.text}
        ...
        placeholder="输入步骤说明..."
        rows={2}
        className="flex-1 resize-none text-sm outline-none bg-transparent text-gray-800 placeholder:text-gray-400 mt-1"
      />
      <button
        type="button"
        onClick={() => onChange(step.id, 'requiresMedia', !step.requiresMedia)}
        title="是否需要拍照/录像"
        className={`flex items-center gap-1 mt-1.5 px-2 py-1 rounded text-xs font-medium shrink-0 transition
      ...
      {total > 1 && (
        <button type="button" onClick={() => onDelete(step.id)}
          className="mt-1.5 p-1 text-gray-300 hover:text-red-500 transition shrink-0">
          <X size={14} />
        </button>
      )}
    </div>
...
function FormModal({
  ...
  const [steps, setSteps]           = useState<Step[]>(initial?.steps ?? [{ id: uid(), text: '', requiresMedia: false }]);
  const [dragIndex, setDragIndex]   = useState<number | null>(null);
  const [dragOverIndex, setDragOverIndex] = useState<number | null>(null);
  const [error, setError]           = useState('');
  const [saving, setSaving]         = useState(false);

  function addStep() { setSteps((p) => [...p, { id: uid(), text: '', requiresMedia: false }]); }
  ...
  async function handleSave() {
    ...
    if (steps.some((s) => !s.text.trim())) { setError('步骤说明不能为空'); return; }
    setSaving(true);
    try {
      await onSave({ title: title.trim(), description: description.trim(), steps });
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium text-gray-700">步骤列表 <span className="text-gray-400 font-normal">（可拖拽排序）</span></label>
...
function AssignModal({
  card, workers, onSave, onClose,
}: {
  card: Flashcard;
  workers: Worker[];
  onSave: (workerIds: string[]) => Promise<void>;
  onClose: () => void;
}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-5 py-4 space-y-2 max-h-72 overflow-y-auto">
          {workers.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4">暂无工人，请先邀请工人加入</p>
          ) : (
            workers.map((w) => {
              const checked = selected.has(w.id);
              return (
                <label key={w.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-gray-50 cursor-pointer transition">
                  <input type="checkbox" checked={checked} onChange={() => toggle(w.id)} className="w-4 h-4 rounded accent-blue-600" />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-xs bg-white rounded-2xl shadow-xl p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-sm text-gray-500 mb-5">
          确认删除《<span className="font-medium text-gray-700">{title}</span>》？此操作不可撤销。
        </p>
        <div className="flex gap-3">
...
export default function FlashcardsPage() {
  ...
  async function handleAssign(workerIds: string[]) {
    ...
    setCards((prev) => prev.map((c) =>
      c.id === assignTarget.id ? { ...c, assignedCount: c.assignedCount + (res.created ?? 0) } : c
    ));
  ...
  return (
    <div className="space-y-5">
            ...
            return (
              <div key={card.id} className="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4 flex items-start gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {card.description && (
                    <p className="text-xs text-gray-500 mt-1 line-clamp-1">{card.description}</p>
                  )}
                ...
                <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
                  <button onClick={() => setFormTarget(card)}
                    className="h-8 px-3 rounded-lg text-xs font-medium text-gray-600 hover:bg-gray-100 transition">
                    <Edit2 size={13} className="inline mr-1" />编辑
                  </button>
                  <button onClick={() => setAssignTarget(card)}
                    className="h-8 px-3 rounded-lg text-xs font-medium text-blue-600 hover:bg-blue-50 transition">
                    <Send size={13} className="inline mr-1" />下发
                  </button>
                  <button onClick={() => setDeleteTarget(card)}
                    className="h-8 px-3 rounded-lg text-xs font-medium text-red-500 hover:bg-red-50 transition">
      ...
      {assignTarget && (
        <AssignModal card={assignTarget} workers={workers} onSave={handleAssign} onClose={() => setAssignTarget(null)} />
      )}
      {deleteTarget && (
        <DeleteConfirm title={deleteTarget.title} onConfirm={handleDelete} onClose={() => setDeleteTarget(null)} />
      )}
    </div>
  );
}
